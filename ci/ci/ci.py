...
async def update_envoy_configs(db: Database, k8s_client):
    ...
    for proxy in ('gateway', 'internal-gateway'):
        cds_config, rds_config = await generate_envoy_configs(db, proxy, live_namespaces)
        configmap_name = f'{proxy}-xds-config'
        configmap = await k8s_client.read_namespaced_config_map(
            name=configmap_name,
            namespace=DEFAULT_NAMESPACE,
        )
        configmap.data['cds.yaml'] = yaml.dump(cds_config)
        configmap.data['rds.yaml'] = yaml.dump(rds_config)
