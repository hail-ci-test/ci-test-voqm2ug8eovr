...
def gateway_default_host(service: Service) -> dict:
...
def gateway_internal_host(services_per_namespace: Dict[str, List[Service]]) -> dict:
    return {
        '@type': 'type.googleapis.com/envoy.config.route.v3.VirtualHost',
        'name': 'internal',
...
def write_to_arg_file_or_stdout(argv_index: int, contents: str):
    if argv_index < len(sys.argv):
        with open(sys.argv[argv_index], 'w', encoding='utf-8') as out:
            out.write(contents)
    else:
...
if __name__ == '__main__':
    ...
    clusters_config = yaml.dump(create_cds_response(services, {}, proxy))
    write_to_arg_file_or_stdout(3, clusters_config)
    routes_config = yaml.dump(create_rds_response(services, {}, proxy))
    write_to_arg_file_or_stdout(4, routes_config)
